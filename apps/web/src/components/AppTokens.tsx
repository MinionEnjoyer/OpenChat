...
export function AppTokens({ label, input }: { label: React.CSSProperties; input: React.CSSProperties }) {
  ...
  return (
    <div>
      ...
      <div className="settings-inline-form" style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
      ...
      {tokens.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {tokens.map((t) => (
            <div className="settings-list-row" key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 6, background: 'var(--input-bg)' }}>
